...
/* creates the main object file (.ob) */
void create_ob_file(const char *filename, InstructionNode *inst_head, DataNode *data_head, int icf, int dcf) {
    ...
    len = strlen(filename);
    ob_filename = (char *)malloc(len + 4); /* for ".ob\0" */
    if (ob_filename == NULL) {
    ...
    /* print data image (4 bytes per line format) */
...
void create_ent_file(const char *filename, SymbolNode *sym_head) {
    ...
    /* check if there is at least one entry symbol */
    while (curr != NULL) {
        if (curr->is_entry) {
            has_entry = 1;
            break;
    ...
    if (!has_entry) {
        return; /* no entries, skip creating the file */
    }

    len = strlen(filename);
    ent_filename = (char *)malloc(len + 5); /* for ".ent\0" */
...
/* creates the externals file (.ext) */
void create_ext_file(const char *filename, ExternNode *ext_head) {
    ...
    if (ext_head == NULL) {
        return; /* no externals, skip creating the file */
    }

    len = strlen(filename);
    ext_filename = (char *)malloc(len + 5); /* for ".ext\0" */
...
/* appends a new node to the external usages list */
...
/* frees the memory of the external list */
