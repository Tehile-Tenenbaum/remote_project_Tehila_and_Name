...
int execute_pass2(const char *filename, SymbolNode *sym_head, InstructionNode *inst_head, ExternNode **ext_head) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        /* position line_remainder at the first token, skipping a label if present */
    ...
    if (first_word[strlen(first_word) - 1] == ':') {
        ...
        line_remainder += strlen(first_word);   /* step past "LABEL:" */
    }
    ...
    if (distance < MIN_IMMED || distance > MAX_IMMED) {
        report_error(file_with_extension, line_number,
                     "Branch distance to '%s' (%ld) exceeds the 16-bit range.",
                     operand, distance);
    } else {
        current_inst->word.machine_code |= ((unsigned long)distance & I_IMMED_MASK);
    }
}
